...
# les fiches techniques et references depuis techniques_data.py. Aucune
# dependance externe. Usage :
...
def load_payloads():
    # Extrait la liste PAYLOADS de seed_payloads.py sans executer le module.
...
UI = {
    ...
    "fr": {
        ...
        "subtitle": "Une bibliotheque navigable de payloads d'injection de prompt pour du red teaming LLM autorise.",
        "disclaimer": "Pour tests autorises, CTF et labs uniquement. Ne visez jamais un systeme sans autorisation.",
        "search": "Rechercher un payload...",
        "f_technique": "Technique",
        "f_objective": "Objectif",
        "f_type": "Type",
        "all": "Toutes",
        "copy": "Copier",
        "copied": "Copie",
...
CSS = """
:root{--bg:#1a1a2e;--bg-alt:#16213a;--panel:#0f3460;--accent:#e94560;--text:#e0e0e0;--muted:#8a94b8;--ok:#3ad29f;--border:#26355c;--mono:"SFMono-Regular","Consolas","Menlo","Monaco",monospace;}
*{box-sizing:border-box;}
body{margin:0;background:var(--bg);color:var(--text);font-family:var(--mono);font-size:14px;line-height:1.5;}
a{color:var(--accent);text-decoration:none;}a:hover{text-decoration:underline;}
.topbar{display:flex;align-items:center;gap:18px;padding:12px 24px;background:var(--bg-alt);border-bottom:1px solid var(--border);position:sticky;top:0;z-index:10;flex-wrap:wrap;}
...
def render(lang, payloads):
    ...
    out.append('<span class="muted">cheatsheet</span>')
    out.append('<div class="right">')
    out.append('<a href="%s">%s</a>' % (esc(u["other"]), esc(u["other_label"])))
    out.append('<a href="%s" target="_blank" rel="noopener">%s</a>' % (REPO_URL, esc(u["repo"])))
    out.append("</div></div>")
    ...
    for tech in techniques:
        ...
        for name, content, technique, objective, itype in payloads:
            ...
            out.append('<div class="pl" data-technique="%s" data-objective="%s" data-type="%s" data-search="%s">'
                       % (esc(technique), esc(objective), esc(itype), search_blob))
            out.append('<div class="pl-top"><span class="pl-name">%s</span><span>' % esc(name))
...
def main():
    ...
    files = {"en": "index.html", "fr": "index.fr.html"}
